...
class _RecentTestListViewState extends State<RecentTestsView> with AutomaticKeepAliveClientMixin {
        ...
        children: [
          ...
          Expanded(
            child: Container(
              ...
              child: StreamBuilder(
                  stream: FirestoreDatabase(uid: context.read<SessionCubit>().currentUser.value!.documentId).allTestsStream(context.read<SessionCubit>().currentUser.value!.organizationId),
                  builder: (context, AsyncSnapshot<List<Map<String,dynamic>>> snapshot) {
                    if (snapshot.hasData) {
                      final tests = snapshot.data!
                          .map((doc) {
                            debugPrint("autoInterpretations : ${doc['autoInterpretations']}");
                      ...
                      return tests.isEmpty
                          ...
                          itemBuilder: (buildContext, index) =>
                              AllTestCard(testDetails: tests[index],margin: EdgeInsets.all(16.w),));
                    } else {
                      return const Center(
